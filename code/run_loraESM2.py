...
train_path_pickle_cdr3 = os.path.join(root, f'esm2_feature_cdr3_train')
train_path_pickle_peptide = os.path.join(root, f'esm2_feature_peptide_train')
test_path_pickle_cdr3 = os.path.join(root, f'esm2_feature_cdr3_test')
test_path_pickle_peptide = os.path.join(root, f'esm2_feature_peptide_test')
...
def encode_batch(model, tokenizer, df, device):
    ...
    for _, seq in tqdm(df, desc="Encoding CDR3"):
        ...
        with torch.no_grad():
            ...
            hidden_states = outputs.hidden_states[33]  # 第33层隐藏状态
            ...
            for i in range(hidden_states.size(0)):
                ...
                start = 1#去头
                end = valid_len - 1 if valid_len > 2 else valid_len# 去尾
...
def encode_and_save(data, cdr3_path, peptide_path):
    ...
    if args.use_base_model:
        model = AutoModelForMaskedLM.from_pretrained(args.base_model_path).to(device).eval()
    else:
    ...
    for batch in peptide_batches:
        peptides_encoding = encode_batch_peptide(batch)
...
print('Encoding and saving train data features...')
encode_and_save(train_data, train_path_pickle_cdr3, train_path_pickle_peptide)
